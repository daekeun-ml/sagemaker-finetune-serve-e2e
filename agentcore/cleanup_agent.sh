#!/usr/bin/env bash
# cleanup_agent.sh — AgentCore 로컬/클라우드 리소스 정리 (과금 중단).
#
# 정리 대상:
...
#
# 🔴 dev 서버 종료는 kill <pid> 로 정밀하게(‘pkill -f agentcore dev’는 실행 셸까지 죽일 수 있음).
set -uo pipefail
...
if [ "$KILLED" = "0" ]; then
  log "  (실행 중인 dev 프로세스 없음 — 종료할 것 없음)"
else
...
log "완료. 🔴 SLM endpoint는 별개 — 99_cleanup.ipynb 또는 SageMaker 콘솔에서 삭제하세요."
