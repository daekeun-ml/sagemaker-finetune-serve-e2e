#!/usr/bin/env bash
# verify_local.sh — AgentCore 에이전트를 배포 전 '로컬에서' 검증 (dev 서버 기동→추론→종료 자동).
#
...
# 예:
#   bash agentcore/verify_local.sh gemma-extraction-hf-1784954006 us-west-2
#
# 🔴 실측 노하우(이 스크립트가 대신 처리): dev 서버는 setsid + </dev/null 로 백그라운드 기동(터미널
#    점유·stdin 문제 회피), 종료는 kill <pid>로 정밀하게(‘pkill -f agentcore dev’는 실행 셸까지 죽임).
set -uo pipefail

SLM_ENDPOINT_NAME="${1:?usage: verify_local.sh <SLM_ENDPOINT_NAME> [AWS_REGION] [PROMPT]}"
AWS_REGION_IN="${2:-${AWS_REGION:-us-west-2}}"
...
command -v agentcore >/dev/null 2>&1 || { err "agentcore CLI 없음 → bash agentcore/setup_agentcore_cli.sh"; exit 1; }

# --- 프로젝트 폴더 확인 ---
HERE="$(cd "$(dirname "$0")" && pwd)"        # .../agentcore
PROJ_DIR="$HERE/$PROJECT_NAME"
if [ ! -d "$PROJ_DIR" ]; then
  err "에이전트 프로젝트 '$PROJ_DIR' 없음 → 먼저: bash agentcore/create_agent.sh"; exit 1
fi
...
# 포트가 닫힐 때까지 폴링(최대 ~15s) — kill 직후엔 잠시 열려있을 수 있음(오탐 방지)
CLOSED=0
for _ in $(seq 1 8); do
  sleep 2
  code="$(curl -s -o /dev/null -w '%{http_code}' --max-time 3 "http://localhost:${PORT}/ping" 2>/dev/null || echo 000)"
