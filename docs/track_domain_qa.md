...
!!! info "Scope"
    자유서술형 답변을 내는 모델을 만들려는 분을 위한 **태스크별 실습 코스**(데이터 준비 → 학습 → 배포 → 평가 → 정리를 한 태스크로 완주하는 단위) 소개입니다(`tracks/04_domain_qa`). "질문(+참고 문서)을 주면 사람이 쓴 것 같은 답을 돌려준다"가 목표라면 이 코스가 맞습니다.
    ...
    **다루지 않는 것**: 검색기를 붙이는 RAG는 이 코스 위에 얹는 별도 작업이고(아래 오개념 참고), 긴 문서 요약은 [요약 코스](track_summarization.md)입니다.
...
이 코스의 노트북은 **9개**입니다. 텍스트 코스의 표준 세트와 같고, **`02a_train_grpo_sagemaker`만 없습니다**(`02a`를 갖는 것은 추출·분류 코스 둘뿐이라 그 두 코스만 10개입니다).
