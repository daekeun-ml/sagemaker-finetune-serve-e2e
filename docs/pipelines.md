...
!!! info "Scope"
    노트북 대신 **파이썬 스크립트로 코스를 실행하는 방법**을 다룹니다. 검증된 코스를 다시 돌릴 때, CI, 무인 실행, 결과 재현이 대상입니다.
    처음 배우는 중이거나 중간 산출물을 눈으로 보고 싶다면 노트북이 맞습니다 — 두 경로의 선택 기준은 [실행 runbook](RUN_E2E.md#두-가지-실행-방법)에 있습니다.
    학습 내용 자체(LoRA·하이퍼파라미터)는 [파인튜닝](03_finetuning.md), 서빙 엔진 선택은 [서빙 컨테이너](05_serving_containers.md)가 담당합니다.
