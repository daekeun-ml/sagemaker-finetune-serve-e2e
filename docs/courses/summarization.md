...
!!! warning "법안 원문의 이중 백틱이 노트북 출력을 삼킵니다"
    ...
    `common/display_utils.py`가 입력·예측을 모두 `<pre>`로 감싸 막아 두었습니다. `html.escape()`로는 막을 수 없습니다(`<`, `>`, `&`만 변환하고 백틱은 통과시킵니다). 직접 셀을 쓸 때도 같은 규칙을 지키세요.
...
이 코스의 노트북은 **9개**입니다. 텍스트 코스 표준 세트에서 `02a_train_grpo_sagemaker`만 빠져 있습니다(`02a`를 갖는 것은 추출·분류 코스 둘뿐이라 그 두 코스만 10개입니다).
