...
1. **최신 모델 반영** — Gemma는 분기마다 릴리스가 갱신됩니다. 이 경로에서는 `MODEL_SIZE`(E2B/E4B/12B/26B-A4B/31B) 또는 `MODEL_ID` env 하나만 바꾸면 승급이 끝납니다(`common/config.py`의 `GEMMA4_PRESETS`, `DEFAULT_MODEL_ID`). 반면 JumpStart는 큐레이션 목록에 오를 때까지 기다려야 할 수 있습니다.
...
`/opt/ml/model` **전체가** `model.tar.gz`로 올라가므로, epoch마다 쌓인 체크포인트가 업로드 시간을 늘립니다(= 한도를 잡아먹습니다). 서빙은 머지된 루트만 읽으므로 `SFTConfig(save_total_limit=1)`로 1개만 남깁니다(체크포인트 3개 = 0.7GB, 전부 서빙에 불필요).
...
??? question "오개념 — “학습이 곧 배포 아닌가요?”"
    ...
    SageMaker 추론에는 Real-time / Serverless / Asynchronous / Batch Transform 네 옵션이 있고 **Serverless는 GPU가 없어 LLM에 부적합**합니다. 이 kit은 real-time endpoint를 씁니다 — [왜 Real-time인가](04_sagemaker_inference.md#왜-real-time인가--추론-4옵션-비교).
