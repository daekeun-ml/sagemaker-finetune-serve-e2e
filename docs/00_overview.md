...
| 방법 | 적합한 경우 | 확인할 문서 |
|---|---|---|
| 노트북 | 처음 실행하거나 중간 결과를 확인할 때 | [E2E 실행 가이드](RUN_E2E.md) |
| Python 파이프라인 | 검증된 코스를 반복 실행하거나 CI에서 사용할 때 | [파이프라인 실행](pipelines.md) |
| 파이프라인 `--dry-run` | AWS 리소스를 만들지 않고 실행 계획을 확인할 때 | [파이프라인 실행](pipelines.md) |
...
엔진별 차이와 호환성 문제는 [서빙 컨테이너](05_serving_containers.md), 학습 설계는 [파인튜닝](03_finetuning.md)에서 다룹니다.
...
| [02 합성 데이터](02_synthetic_data.md) | 시드 변환, 합성 데이터와 held-out 분리 | `01_data_and_synthetic` | `common/synth/`, `track_data.py` |
| [03 파인튜닝](03_finetuning.md) | SFT, GRPO, LoRA, QLoRA와 모델 저장 | `02`, `02a` | `scripts/train.py`, `scripts/train_grpo.py` |
| [04 SageMaker AI 추론](04_sagemaker_inference.md) | Endpoint 배포, 호출, 평가와 정리 | `03`, `04`, `99` | `common/aws_utils.py` |
| [05 서빙 컨테이너](05_serving_containers.md) | vLLM, SGLang, DJL LMI와 호환성 문제 | `02b`, `03` | `common/dlc.py`, `common/model_inspect.py` |
| [06 Agentic loop](06_agentic.md) | Strands, Bedrock과 AgentCore Runtime | `05`, `06` | `agentcore/app.py`, `common/llm_gateway.py` |
| [E2E 실행 가이드](RUN_E2E.md) | 노트북 실행 순서와 단계별 확인 사항 | 전 단계 | 코스별 노트북 |
| [Python 파이프라인](pipelines.md) | 코스 자동 실행, 재개와 정리 | 전 단계 | `pipelines/` |
| [SageMaker Managed MLflow](mlflow.md) | 실험 설정, 학습 지표와 평가 결과 추적 | 학습과 평가 | `common/mlflow_utils.py` |
| [속도 측정](benchmark.md) | TTFT, TPOT, ITL과 E2EL 측정 | 배포 이후 | `pipelines/run_benchmark.py` |
...
다음 단계는 [시작하기](getting_started.md) 또는 [E2E 실행 가이드](RUN_E2E.md)입니다.
