...
!!! warning "빠르게 바뀌는 값"
    DLC 이미지 태그와 `OPTION_*`/`SM_*` 키 이름과 SDK 버전과 엔진 지원 매트릭스, 리전, GA 상태는 분기마다 바뀝니다.
...
**이 프로젝트는 `SERVING_ENGINE` env로 vLLM DLC(기본) / SGLang DLC / DJL LMI 셋 중 하나를 골라 배포합니다. 셋 다 연속 배칭 + OpenAI 호환이라 호출 코드가 같습니다.**
...
5. **real-time endpoint는 삭제 전까지 시간당 과금됩니다.** 어떤 컨테이너를 골랐든 마찬가지입니다. 자세히는 [비용과 cleanup](#비용과-cleanup)을 보세요.
...
- 셋 다 "LLM serving"으로 설명되어 있어 **각 구성요소의 관계**를 파악하기 어렵습니다.
- 설정을 건드리지 않았는데 endpoint가 `Failed`로 끝나고, 로그에는 `ping health check` 실패만 남습니다.

가장 흔한 혼동은 **"엔진"과 "서빙 컨테이너"를 같은 레이어로 보는 것**입니다.
...
`DJL LMI`는 AWS 관리형 serving container이며, **내부 엔진을 바꿀 수 있다**는 점이 특징입니다. 예를 들어 `OPTION_ROLLING_BATCH=vllm`으로 지정하면
그 안에서 vLLM 엔진이 동작합니다.
...
그림의 요점은 **LMI가 엔진이 아니라 stack**이라는 점입니다.
모델 서버는 [DJLServing](https://github.com/deepjavalibrary/djl-serving)이고, vLLM은 그 아래에서 동작하는 backend 중 하나입니다.
(LMI 문서는 `backend` = Python Engine + 추론 라이브러리로 정의합니다.)

여기서 갈리는 것은 "백엔드를 바꿀 때 무엇이 따라 바뀌는가"입니다.

- **백엔드를 바꿔도 그대로인 것**: `/ping`, `/invocations`, 요청 큐(`job_queue_size`)는 계속 DJLServing이 담당합니다. 우리가 손대는 것은 그림 오른쪽 스니펫 네 줄 수준입니다.
- **백엔드를 바꾸면 함께 바뀌는 것**: **연속 배칭은 백엔드 쪽에 있습니다.**
  스니펫의 `option.rolling_batch`가 바로 "어느 백엔드로 연속 배칭할지"를 고르는 키이므로, `vllm` → TRT-LLM으로 바꾸면 배칭 구현도 바뀌고 PagedAttention, 연속 배칭의 성능 특성도 그 순간 달라집니다.

설정 키의 이름 규칙은 두 갈래입니다.

- `serving.properties`의 `option.<키>`는 `OPTION_<키>` env와 1:1입니다(`option.rolling_batch` ↔ `OPTION_ROLLING_BATCH`).
- `option.`으로 시작하지 않는 서버 설정은 `SERVING_` 접두사를 씁니다(`job_queue_size` → `SERVING_JOB_QUEUE_SIZE`).
...
- 이 프로젝트가 안 쓰는 이유: 학습 artifact를 `model_data`로 넘겨 **SageMaker AI가** `/opt/ml/model`에 풀게 하고 `HF_MODEL_ID=/opt/ml/model`을 주기 때문입니다.
...
!!! warning "그림은 스냅샷입니다"
    ...
    HuggingFace Accelerate는 *maintenance* 상태로 표시됩니다. 이 프로젝트가 쓰는 이미지는 `.env`의 `LMI_IMAGE_URI`가
    결정하므로, 버전은 그림이 아니라 [현행 태그](#이-프로젝트-env의-이미지-고정값)를 보세요.
...
| SageMaker AI 통합 | native(DLC) | BYOC 필요(`/ping`+`/invocations`) 또는 어댑터 | native(HF DLC) |
...
3. **SageMaker AI 규약을 누가 처리하는가**: 세 DLC 모두 `/ping`, `/invocations`, 모델 로딩이 **이미 구현**돼 있어
   ...
   `SM_VLLM_*` env를 `--` CLI 플래그로 바꿔(`SM_VLLM_MAX_MODEL_LEN` → `--max-model-len`) 서버를 띄웁니다.
    ...
    !!! note "직접 맞춰야 하는 경우는 BYOC뿐입니다"
        ...
        OpenAI 서버 앞단에 adapter를 두어야 합니다. 이 프로젝트는 DLC를 쓰므로 해당하지 않습니다.
...
| 모델 artifact | S3의 `model.tar.gz`를 풀어 **`/opt/ml/model`**에 넣음(읽기 전용) | SageMaker AI |
...
**둘째, `/invocations` 60초.** 생성 길이의 상한을 사실상 여기서 받습니다. 이 프로젝트 실측(L4, 약 40ms/토큰)은 한도의 3분의 1 수준이었습니다([max_tokens 절단](#max_tokens-절단과-finish_reason)).
...
- SGLang DLC는 `--model-path`를 생략하면 기본값이 `/opt/ml/model`, 포트 8080, 호스트 `0.0.0.0`입니다.
...
| 형태 | LLM/SLM 적합성 |
|---|---|
| Real-time | ✅ 적합: 이 프로젝트의 기본(상시 저지연). GPU 인스턴스 |
...
- **LMI 태그 형태(예시)**: `djl-inference:0.XX-lmiXX-cuXXX` 계열입니다.
  정확한 `repository`/`tag`는 버전마다 다릅니다.
  **[available_images 페이지](https://aws.github.io/deep-learning-containers/reference/available_images/)에서 확인**한 뒤 env로 주입하세요.

이미지를 해석하는 경로는 두 가지입니다.

1. **SDK resolve**: `image_uris.retrieve(framework="djl-lmi"/"vllm" 등, region, version=...)`를 호출하면 SDK가 계정, 리전, 태그를 조립해 줍니다.
   ([API 문서](https://sagemaker.readthedocs.io/en/stable/api/generated/sagemaker.core.image_uris.html#sagemaker.core.image_uris.retrieve))
   다만 **SDK 버전에 매인 태그 목록**이라 최신보다 늦을 수 있습니다.
   경로는 SDK v3에서 `sagemaker.core.image_uris.retrieve`이고, v2 경로(`sagemaker.image_uris`)는 fallback입니다.
2. **직접 지정**: 위 패턴으로 URI를 **직접** 만들고 env로 override하는 방식입니다. available_images가 갱신되어도 코드를 고치지 않고 태그만 교체하면 됩니다.

**`image_uris.retrieve`에 넘기는 framework 문자열과 지원 `version` 값은 SDK 버전과 시점마다 다르므로 실행 전 재확인이 필요합니다.**

- `common/dlc.py`의 기본 repository/tag 값은 `# TODO verify` 예시일 뿐입니다. available_images에서 현행 값을 확인해 env로 덮어쓰세요.
- 계정 ID와 URI 패턴은 하드코딩되어 있지만, 이는 AWS 공개 레지스트리 값이라 시크릿이 아닙니다. 반면 고객 계정 ID, role, bucket은 여전히 하드코딩하면 안 됩니다.

### 이미지 해석 우선순위: common/dlc.py

이 프로젝트는 **env를 우선하는 직접 지정** 전략을 택했고, SDK resolve와 패턴 조립을 fallback으로 둡니다. 서빙 이미지는 `resolve_serving_image(region, engine)`이 엔진별 해석 함수로 위임합니다.

| 엔진 | 해석 함수 | 완전 URI env(최우선) | 버전 env | fallback |
|---|---|---|---|---|
...
서빙 이미지와 별개로 해석되는 helper가 둘 더 있습니다.

- `resolve_training_image()`(학습): `DLC_IMAGE_URI` → `DLC_REPOSITORY`+`DLC_TAG` → retrieve 순
- `resolve_inference_image()`(범용 추론): `INFER_DLC_IMAGE_URI` → `INFER_DLC_REPOSITORY`+`INFER_DLC_TAG` 순
...
이 프로젝트의 `.env`는 **이미지를 리전 포함 완전 URI로 하드코딩**해 둡니다. 무엇이 쓰이는지 한눈에 보이고, SDK의 `image_uris.retrieve` 추측을 우회합니다.
...
ECR 실조회(763104351884, us-west-2) 당시의 최신 태그는 다음과 같았습니다. 태그를 손으로 조립할 때 걸리는 곳이 두 군데 있습니다.

- **SGLang DLC만 우분투 버전이 다릅니다**(24.04).
- **LMI 태그는 버전 축이 둘입니다.** 앞의 `0.36.0`은 djl-serving 버전이고, gemma-4 지원 여부를 가르는 것은 번들 vLLM을 결정하는 **`lmi27.0.0`** 쪽입니다.

!!! danger "LMI는 완전 URI를 지워선 안 됩니다: fallback이 lmi26으로 떨어집니다"
    `LMI_IMAGE_URI`를 주석 처리하면 `dlc.resolve_lmi_image()`가 `LMI_VERSION`(기본 `0.36.0`)으로 SDK의 `image_uris.retrieve(framework="djl-lmi", ...)`에 위임하고, **태그 조립은 SDK의 버전 표가** 합니다. 이 프로젝트에 설치된 SDK의 `djl-lmi.json`은 `0.36.0 → 0.36.0-lmi26.0.0-cu130`으로 매핑하므로, fallback으로 얻는 이미지는 **lmi26 = gemma-4를 로드하지 못하는 이미지**입니다. `0.36.0`이라는 djl-serving 키는 lmi 축을 고정해 주지 않습니다.
...
리전을 옮길 때는 `AWS_REGION`과 위 URI의 리전을 함께 바꿉니다(이미지는 리전별 ECR에서만 pull됩니다).

- 리전을 자주 옮긴다면 vLLM, SGLang 줄을 **주석 처리**하세요. 그러면 코드가 `AWS_REGION`으로 URI를 자동 조립합니다(`VLLM_DLC_VERSION` 등으로 버전만 지정).
- LMI 줄은 예외입니다. fallback이 lmi26으로 떨어집니다(위 주의 참고).
- 해석 결과는 `dlc.serving_image_table(region)`으로 **세 엔진**을 한 번에 확인할 수 있고, `03_deploy_endpoint` 노트북이 이를 출력합니다.

### 서빙 env → CLI 플래그 변환 규칙

vLLM, SGLang DLC의 `sagemaker_entrypoint.sh`는 **접두사를 떼고 소문자화 + `_`→`-`** 해서 그대로 엔진 CLI 플래그로 넘깁니다.
...
별도 **whitelist는 없습니다**. 엔진이 지원하는 flag라면 이 규칙으로 전달할 수 있습니다.
LMI만 규약이 달라 `OPTION_*`(예 `OPTION_ROLLING_BATCH=vllm`)를 쓰며, 이 값들은 vLLM `EngineArguments`로 pass-through됩니다.

---

## 이 프로젝트의 배포 경로: 03_deploy_endpoint

이 프로젝트는 "하나만 쓰라"고 강요하지 않습니다. **vLLM DLC를 기본으로**, SGLang DLC와 DJL LMI를 나란히 제공합니다.
배포 후 호출 방식은 **동일**합니다. 어떤 컨테이너를 사용해도 `sagemaker-runtime`으로 호출합니다.
...
**셋 다 연속 배칭 + OpenAI 호환(`messages`) + 스트리밍을 지원합니다**. 그래서 엔진을 바꿔도 호출 코드(04 평가와 05 agentic)가 그대로 돕니다.

`config.SERVING_ENGINE`의 기본값은 모델 프리셋의 `servable_engine`(전 사이즈 `vllm`)입니다. 프리셋은 `MODEL_SIZE` env로 고르는 **E4B(기본), 12B, 26B-A4B** 세 가지입니다.
...
정확한 `OPTION_*`/`SM_*` 키 이름과 기본값은 컨테이너 버전마다 다릅니다.
...
??? question "오해: “transformers(HF Inference DLC) 경로는 왜 없나요?”"
    **의도적으로 제외했습니다.** `code/inference.py` 핸들러로 서빙하면 **단건 처리**라 연속 배칭이 없고, [SageMaker HuggingFace Inference Toolkit](https://github.com/aws/sagemaker-huggingface-inference-toolkit)이 응답을 완성본으로 버퍼링해 **토큰 스트리밍도 불가**합니다([응답 스트리밍](#응답-스트리밍-vllm-경로에서의-지원-여부)).
    E4B가 vLLM으로 못 뜬다고 알려졌을 때의 우회로였는데, 그 원인이 checkpoint였음이 밝혀져([KV-shared 복원](#e계열-kv-shared-dead-weight-복원)) 더는 필요하지 않습니다.
...
- 배포할 때 완전 URI env(`VLLM_IMAGE_URI`)에 **본인이 빌드해 푸시한 vLLM 이미지 URI**를 넣으면 프로젝트의 배포 코드를 그대로 재사용할 수 있습니다.

호출은 컨테이너와 무관하게 동일합니다. `common/aws_utils.py`가 세 함수로 나눠 담당합니다.

| 함수 | 담당 |
|---|---|
| `invoke_sagemaker_chat()` | OpenAI 호환 `messages` 스키마 |
...
??? question "오해: “IN_PROCESS로 gemma를 초경량 검증하면 되지 않나?”"
    **안 됩니다(생성형 LLM 미지원).** IN_PROCESS 서버는 `model=<HF id>`를 받으면 내부적으로 **`transformers.pipeline` 또는 `SentenceTransformer`(임베딩)로만** 로드를 시도합니다(SDK 3.16.0 소스 실측: `sagemaker/serve/model_server/in_process_model_server/app.py`). 즉 분류와 임베딩 같은 경량 모델 전용입니다.
    gemma-4는 멀티모달(오디오 포함)이라 pipeline이 `AnyToAnyPipeline`으로 잡혀 `librosa` 등을 요구하고, 임베딩 모델도 아니라 `SentenceTransformer` fallback도 실패합니다(`UnboundLocalError`).
...
- **DJL LMI + LOCAL_CONTAINER**: 컨테이너와 마운트까지는 됩니다(모델을 `model_path/code/`에 **실파일**로 둬야 마운트됩니다: 심링크는 컨테이너 안에서 깨집니다).
  당시 `weights not initialized: layers.24~41...k_norm`로 엔진 초기화가 실패했는데, 원인은 LMI가 아니라 우리가 넘긴 checkpoint였습니다
...
- 참고: **HF PyTorch Inference DLC + `model_server=MMS` + LOCAL_CONTAINER**는 E4B에서 성공했습니다(실측: 로드 + `/invocations` 응답 확인).
  다만 이 프로젝트는 transformers 단건 서빙 경로를 서빙 선택지에서 제외했습니다(연속 배칭과 스트리밍 없음).

### 결론: 이 프로젝트의 gemma-4 검증 경로

- **로컬 검증**: SDK 로컬 모드(IN_PROCESS/LOCAL_CONTAINER)를 **기본 경로로 쓰지 않습니다.**
  대신 **`02b_local_serve`**에서 `vllm serve`로 실제 엔진을 띄워 확인합니다(`scripts/serve_local_vllm.sh`).
...
**이 프로젝트의 처리.** `scripts/train.py`, `train_grpo.py`의 `_revive_kv_shared_from_base()`가 저장 직전에 base
checkpoint에서 그 54개를 읽어 `save_pretrained(state_dict=...)`로 함께 저장합니다. 모델 객체엔 해당 모듈이
...
| checkpoint | 저장 키 | vLLM 로드 |
|---|---|---|
| 복원 전(`save_pretrained` 그대로) | 665 | ❌ 실패: `weights not initialized ...k_norm` |
| 복원 후(이 프로젝트) | 719 = 원본과 동일 | ✅ 성공: 로드 + 정상 생성 |

**참고: vLLM issue [#44788](https://github.com/vllm-project/vllm/issues/44788)**(OPEN 상태)은
바로 이 현상입니다. 이슈 제목이 "Gemma 4 models with KV sharing"이라 "E계열은 vLLM 불가"로 읽히기 쉬우나,
두 번째 코멘트가 `save_pretrained` 왕복 후에만 재현됨을 보여줍니다. 원본 checkpoint는 그 54개를 모두 갖고 있습니다
...
**모델 크기가 원인이 아닙니다.** vLLM 기본 `max_num_seqs=256`이 실습 규모에 비해 커서 sampler buffer가 GPU 메모리를 넘깁니다. 이 프로젝트는 **32**로 낮춰 두었고, GPU를 바꿀 필요는 없습니다.

아래 수치는 vLLM 0.26.0, `ml.g6.2xlarge` 실측이며, 엔진 버전이 바뀌면 배정값도 달라집니다.
...
gemma-4의 vocab이 **262,144**로 크기 때문에, 동시 시퀀스 기본값 256이면 샘플러 logits 버퍼 하나가 256 MiB입니다.
flashinfer가 `torch.empty_like`로 사본을 하나 더 만들므로 실제로는 **512 MiB**가 필요합니다.

### 메모리 예산: L4 22.9GB 실측

`ml.g6.2xlarge`(L4 22.9GB) 기준 한도 = `21.96 × 0.92 = 20.21 GiB`입니다(아래 표는 vLLM 0.26.0 실측이며, 엔진 버전이 바뀌면 KV 배정값이 달라집니다).
...
같은 의미의 설정이 엔진마다 다른 키를 씁니다.

??? info "더 읽을 거리: 키 이름 근거"
    - SGLang 플래그: [`server_args.py` 소스](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/server_args.py)
    - LMI 키와 vLLM pass-through 동작: [LMI vLLM user guide](https://docs.djl.ai/master/docs/serving/serving/docs/lmi/user_guides/vllm_user_guide.html)
...
세 엔진 중 무엇을 골라도 같은 호출로 알맞은 키가 나옵니다(LMI는 `OPTION_ROLLING_BATCH=vllm`과 `OPTION_TENSOR_PARALLEL_DEGREE='max'` 관용구까지 자동).
...
이전 버전 문서는 "E4B는 스트리밍 불가"라고 썼는데, 그것은 **HF PyTorch Inference DLC를 쓰던 시절**의 결론입니다. 서빙 경로가 vLLM/SGLang/LMI 셋으로 바뀐 뒤 E4B에서 토큰 스트리밍이 정상 동작함을 실측으로 확인했습니다.
...
그래서 청크를 받는 즉시 `json.loads`하면 `JSONDecodeError`가 납니다. **버퍼에 모아 `\n\n`(SSE 이벤트 구분자) 단위로만 잘라 파싱**해야 합니다.
`common/aws_utils.stream_sagemaker_chat()`이 이 처리를 담당합니다.
...
- **켤 만한 것**: 요약과 도메인 QA 같은 긴 자유서술 → 이 프로젝트는 두 코스에서 `STREAM = True`가 기본입니다(`_stream_default()`가 `eval_kind`로 판정).
- **끄는 게 맞는 것**: 추출(JSON), 분류(라벨) → 응답이 **완성돼야 파싱/사용 가능**하고 애초에 짧습니다. agentic tool도 완성값을 반환해야 Claude가 소비합니다. → `STREAM = False` 기본.

### 스트리밍이 개선하지 않는 것

**전체 생성 시간과 throughput(동시 처리량)은 그대로입니다**. 첫 토큰 체감만 줄입니다.
위 실측에서도 완료 시각은 15.9s vs 16.2s로 사실상 같았습니다. 동시 처리량은 연속 배칭과 [max_num_seqs 기본값](#24gb-gpu-cuda-oom-max_num_seqs-기본값)이 결정합니다.
...
응답이 짧게 끝났다면 **`finish_reason`으로 정상 종료와 절단을 구분하세요.** `length`는 절단, `stop`은 모델의 정상 종료를 뜻합니다. 아래는 요약 코스 endpoint(입력 5,996자)의 실측값입니다.
...
**원인 1: `streaming=True`가 매번 다시 받습니다(50초).** `load_dataset(..., streaming=True)`는 **로컬 디스크에 캐시하지 않습니다.**
cord-v2는 이미지가 parquet에 내장돼 있어 **첫 row 하나를 꺼내는 데 23초**가 걸리고, 셀을 다시 실행하면 **또** 그만큼 듭니다(재호출 24초).
...
**원인 2: 남은 21초는 정상이고, 대신 응답이 잘리고 있었습니다.** 병목은 **생성 토큰 수**입니다(실측 **약 40ms/토큰**, L4).
payload 크기는 무관했습니다. 1,853KB(PNG) → 64KB(축소 JPEG)로 **29배 줄여도 21.0s → 20.6s**로 그대로입니다.
...
`%store`는 IPython의 **전역** 저장소로, 코스, 커널, 리전을 넘어 값이 유지됩니다.
그래서 멀티모달 코스(`max_model_len=2048`)를 배포한 뒤 요약 노트북을 열면, `%store -r endpoint_name`이 **멀티모달 엔드포인트 이름**을 복구해 옵니다.
...
**진단이 어려운 이유**: 에러가 "context length"를 말하므로 `max_tokens`나 `serve_max_model_len` 설정을 먼저 의심하게 됩니다. 실제 원인은 **다른 endpoint를 호출한 것**입니다.

코스별 고유 키(`ep_<track_key>`)로도 저장하고, 복구할 때 **이 키를 우선**합니다.
...
키가 `ep_extraction` / `ep_classification` / `ep_summarization` / `ep_domain_qa` / `ep_mm_extraction`로 갈리므로 **여러 코스를 병행해도 섞이지 않습니다.**
실제 커널에서 전역을 멀티모달로 오염시킨 뒤 요약 복구를 실행해, 올바른 엔드포인트가 선택됨을 확인했습니다.

같은 이유로 `train_path`는 `%store`를 **아예 쓰지 않고** 코스 로컬 파일(`data/train.jsonl`)을 직접 씁니다. `%store`에 담아야 할 값은 "이 코스 것"임을 이름에 새기세요.
...
아래 내용은 실측 스냅샷이며, vLLM 버전과 config 키와 지원 head는 빠르게 바뀌므로 배포 전 재확인하세요.

**핵심 사실: JumpStart 전용이 아닙니다.** AWS 블로그의 "P-EAGLE on SageMaker"는 **JumpStart 원클릭** 경험을
소개하지만, speculative decoding을 켜는 **config 자체는 container-level 기능**이라 이 프로젝트처럼 JumpStart를 쓰지 않는
...
speculative decoding은 **config 키만 넣는다고 동작하지 않습니다.** target 모델에 맞춰 **학습된 draft head checkpoint**가
반드시 있어야 하며, `parallel_drafting`은 그 목적에 맞게 학습된 head를 추가로 요구합니다.

- **AWS가 공개한 P-EAGLE head**: GPT-OSS-120B/20B, Qwen3-Coder-30B 뿐이고 **Gemma용은 공개돼 있지 않습니다**
  (JumpStart의 Gemma-4-31B-IT용 head는 배포 시 내부 번들되는 artifact로 공개 다운로드 대상이 아닙니다). 이 로스터는 AWS가 언제든 늘릴 수 있으므로 재확인 대상입니다.
...
- **이 프로젝트는 Gemma를 fine-tune합니다.** EAGLE3 head는 target의 hidden-state에 맞춰 학습되므로, base용 head를
...
**이 프로젝트에서의 위치**: `03_deploy_endpoint`는 speculative decoding을 **설정하지 않습니다**(`dlc.serving_env()`가 내보내는 키에 speculative 항목이 없습니다).
위 head 요건 때문에 기본값을 비활성으로 둔 것입니다. 쓰려면 head를 확보해 정합성을 검증한 뒤, `serve_env`에 `SM_VLLM_SPECULATIVE_CONFIG`(LMI는 `OPTION_SPECULATIVE_CONFIG`)를 직접 추가하세요.

---

## 자주 나오는 오해

컨테이너 선택 단계에서 가장 흔한 오해는 엔진과 컨테이너를 경쟁 관계로 보는 것입니다.
...
??? question "오해: “한 번 고르면 영원히 그 컨테이너에 묶이는 것 아닌가요?”"
    **아닙니다.** 이 프로젝트는 이미지 URI를 env(`SERVING_ENGINE` + `*_IMAGE_URI`)로 해석하고, 호출은 `sagemaker-runtime`으로 통일해 두었습니다.
...
- `common/aws_utils.py`: endpoint 호출과 SSE 스트리밍과 CloudWatch 링크(`invoke_sagemaker_chat`, `stream_sagemaker_chat`, `cw_links`)
- `common/display_utils.py`: 잘림과 마크다운 깨짐을 막는 노트북 렌더링(`show_inference`, `stream_inference`)
...
- `tracks/*/scripts/serve_local_vllm.sh`: 배포 전 로컬 `vllm serve` preflight
