...
**Amazon SageMaker AI 추론에는 4가지 옵션(Real-time / Serverless / Asynchronous / Batch Transform)이 있고, LLM/SLM 서빙에는 GPU가 붙는 Real-time이 사실상 유일한 선택입니다. 이 프로젝트는 Real-time endpoint에 vLLM DLC(기본), SGLang DLC, DJL LMI 중 하나를 실어 배포하며, 호출은 `sagemaker-runtime`(Bedrock과 별개 서비스), 정리는 반드시 `99_cleanup`입니다.**

1. **Serverless Inference에는 GPU가 없습니다.** 따라서 LLM/SLM에는 부적합하며, 이것이 이 프로젝트가 Real-time을 선택한 근본 이유입니다([왜 Real-time인가](#왜-real-time인가-추론-4옵션-비교)).
2. **endpoint 호출은 Bedrock 호출과 다릅니다.** endpoint는 `sagemaker-runtime.invoke_endpoint()`, Bedrock Claude는 `bedrock-runtime.converse()`로 부르는 **별개 서비스와 별개 클라이언트**입니다([서비스 경계](#서비스-경계-endpoint--bedrock)).
3. **서빙 컨테이너는 하나로 모든 상황을 해결할 수 없습니다.** 이 프로젝트는 vLLM(기본) / SGLang / DJL LMI 세 경로를 env로 전환합니다([서빙 컨테이너와 DLC 이미지](#서빙-컨테이너와-dlc-이미지)).
...
아래에서 각 문제의 원인과 선택 기준을 순서대로 설명합니다.
...
- 실시간과 서버리스 패널은 `추론 요청` → `추론 결과` 왕복 화살표 하나로 끝납니다.
- 비동기와 배치 패널은 화살표가 세 갈래로 갈라집니다: 즉시 돌아오는 **요청 확인**(접수증), S3로 떨어지는 **결과**, 알림 리스너로 가는 **완료 통보**.
...
- 즉 **응답을 그 자리에서 되받는 것은 Real-time(과 Serverless)뿐**입니다. 이 프로젝트의 04, 05 노트북이 SNS 토픽도 EventBridge 규칙도 없이 `invoke_endpoint()` 한 줄로 끝나는 이유가 여기 있습니다.
...
| **상주 리소스** | 있음(항상 켜짐) | 없음(요청 시 기동) | 있음(auto scaling으로 0까지 축소 가능) | Job 단위(끝나면 종료) |
...
| **LLM/SLM 적합** | ✅ 적합(이 프로젝트) | ❌ 부적합(GPU 없음) | 조건부(긴 생성, 오프라인) | 조건부(대량 오프라인 채점) |
...
??? info "더 읽을 거리: 표의 수치가 나온 곳"
    ...
    - **Serverless**: 요청과 응답 payload가 각각 4 MB, `/invocations` timeout이 1분, `/ping` 응답은 3분 안입니다. **GPU, Multi-Model Endpoint, VPC, Model Monitor는 기능 제외 목록**에 있습니다([Serverless 호출](https://docs.aws.amazon.com/sagemaker/latest/dg/serverless-endpoints-invoke.html)).
...
3. **최소 인스턴스 수**: Async는 인스턴스를 0까지 축소할 수 있어 유휴 비용을 줄일 수 있지만, Real-time은 auto scaling을 설정해도 통상 1대 이상을 유지합니다. 이 때문에 **상시 과금**이 발생합니다.
...
??? question "오해: “Serverless가 제일 싸니까 LLM도 Serverless로 하면 되지 않나요?”"
    **아닙니다.** SageMaker Serverless Inference에는 **GPU가 없습니다**(CPU 전용). Gemma 같은 SLM/LLM은 GPU 없이는 사실상 돌아가지 않거나, 돌아가더라도 실용 불가 수준으로 느립니다.
    ...
    GPU는 단독 항목이 아니라 **기능 제외 목록의 첫 줄**입니다. GPU, Multi-Model Endpoint, VPC 구성과 network isolation, data capture, 다중 production variant, Model Monitor, inference pipeline이 함께 빠집니다. RAM도 최대 6,144 MB(6 GB)까지만 고를 수 있어서, bf16 E4B 가중치 14.23 GiB는 **애초에 올라가지 않습니다**.
...
- 대화형과 실시간 API가 목표이고 트래픽이 꾸준하다면 → **Real-time**을 선택하세요 (이 프로젝트의 선택입니다).
...
|---|---|---|
| **모델** | 단일 모델 배포 / [멀티 모델 배포(MME)](https://docs.aws.amazon.com/sagemaker/latest/dg/multi-model-endpoints.html) / [auto scaling](#auto-scaling과-cloudwatch-관측) | 단일 모델 배포(코스별 endpoint 1개) |
...
- **멀티 모델 배포(MME)는 LLM 서빙에 잘 맞지 않습니다.** GPU 기반 MME는 [Triton Inference Server 컨테이너를 통해서만 지원](https://docs.aws.amazon.com/sagemaker/latest/dg/multi-model-support.html)되고, 지원 GPU 인스턴스도 **p2, p3, g5, g4dn**으로 한정됩니다. vLLM/SGLang DLC는 MME 컨테이너가 아니므로 이 프로젝트는 코스마다 **단일 모델 endpoint를 따로** 띄웁니다.
- **멀티 컨테이너는 단일 SLM 하나를 서빙하는 이 프로젝트에 필요 없습니다.** 한 endpoint에 최대 15개 컨테이너까지 얹을 수 있고, `InferenceExecutionConfig.Mode`가 `Serial`(기본값, inference pipeline으로 순차 실행)인지 `Direct`(`TargetContainerHostname`으로 개별 호출)인지에 따라 호출 방식이 달라집니다([create 문서](https://docs.aws.amazon.com/sagemaker/latest/dg/multi-container-create.html)).
...
- 위 그림의 GPU 예시(P4, G5, G4dn)는 스냅샷입니다. 이 프로젝트는 g5 용량 대기 때문에 **G6(L4)** 세대를 씁니다.
...
이 프로젝트의 `common/aws_utils.py`는 스키마별로 얇은 래퍼 두 개를 제공합니다.

```python
# common/aws_utils.invoke_sagemaker_chat(): OpenAI 호환 chat 스키마 (이 프로젝트의 기본 경로)
...
- template이 빠진 raw 텍스트는 (거부되지 않더라도) 반복과 저품질 출력을 냅니다.
...
이 프로젝트는 이 호출을 `aws_utils.stream_sagemaker_chat()`으로 감쌌습니다.
...
스트리밍 payload 필드와 이벤트 파싱 방식은 컨테이너와 SDK 버전에 따라 달라지므로 **실행 전 재확인**이 필요합니다.
...
| 이 프로젝트의 위치 | 파인튜닝 Gemma 서빙 | agentic reasoning, 합성 데이터 생성 |
...
??? question "오해: “내 endpoint도 Bedrock API로 부르면 되지 않나요?”"
    **아닙니다.** endpoint는 `sagemaker-runtime.invoke_endpoint()`로, Bedrock은 `bedrock-runtime.converse()`로 호출하는 **완전히 다른 서비스**입니다.
    이 프로젝트는 `common/llm_gateway.py`(LiteLLM)로 두 백엔드를 하나의 인터페이스처럼 쓰지만, 내부적으로 각각 올바른 서비스로 라우팅하는 것일 뿐 같은 API를 쓰는 것이 아닙니다.
...
[HF TGI](https://github.com/huggingface/text-generation-inference)와 HF PyTorch Inference DLC는 이 프로젝트의 서빙 선택지에서 제외했습니다. 후자는 transformers 단건 서빙용이라 연속 배칭과 스트리밍이 없습니다(`dlc.resolve_hf_inference_image()`로 남겨 두긴 했습니다).
...
- **chat template**은 서버(vLLM/SGLang/LMI) 또는 `apply_chat_template`에 위임합니다(이 프로젝트의 `common/gemma_format.py`).
- **system role**: Gemma 템플릿에는 전용 system 슬롯이 없는 경우가 많습니다(정확한 동작은 모델별 `tokenizer_config`가 결정합니다). 그래도 이 프로젝트의 코스는 `build_inference_messages(..., system_content=...)`로 만든 system role을 그대로 endpoint에 보내고, 서버의 chat template이 처리합니다. 이 문서에 인용한 실측값도 그 경로에서 나왔습니다.
- **system role 거부 시**: 템플릿이 system role을 거부하는 모델을 만났을 때만 `fold_system_into_user()`로 첫 user 턴에 병합하세요(자동 fallback은 없습니다).
...
**gemma-4 E2B/E4B는 저장 시 KV-shared tensor가 소실되어 vLLM load가 실패합니다. 이 프로젝트는 저장 전에 이를 복원합니다.**
...
| 엔진 | 해석 함수 | 완전 URI env(최우선) | 버전 env | 이 프로젝트 `.env` 실측값(us-west-2) |
|---|---|---|---|---|
...
- **인스턴스**: `config.INFER_INSTANCE_TYPE`을 씁니다. 프리셋 기본값은 E4B → `ml.g5.2xlarge`, 12B / 26B-A4B → `ml.g5.12xlarge`입니다. 이 프로젝트의 `.env`는 g5 용량 대기가 길어 **`ml.g6.2xlarge`(L4 24GB + RAM 32GB)로 override**해 두었습니다.
...
`serving_env()`가 "의미 → 엔진별 키"를 한곳에서 매핑하므로, 노트북은 의미만 넘깁니다. 같은 설정을 세 엔진의 서로 다른 키로 세 번 쓰면 값을 하나 바꿀 때 빼먹기 쉽습니다. 이 프로젝트도 `max_num_seqs`를 vLLM 분기에만 넣고 LMI 분기를 놓쳐 OOM이 재발한 적이 있습니다.
...
이 프로젝트 endpoint 실측값입니다(vLLM 0.26.0, gemma-4 E4B bf16 14.23 GiB, `ml.g6.2xlarge` = L4 22.9GB 가용). KV 캐시를 배정한 뒤 남은 여유가 **0.47 GiB**뿐이었습니다.
...
## Auto scaling과 CloudWatch 관측

- **auto scaling을 설정해도 사용하지 않는 endpoint는 삭제해야 합니다.** Real-time은 **application auto scaling**으로 인스턴스 수를 트래픽에 맞춰 조절할 수 있지만(예: `InvocationsPerInstance`를 target으로 지정), 통상 **최소 1대 이상**을 유지하므로 완전히 0으로 축소되지는 않습니다.
- auto scaling 정책, metric, 최소 인스턴스 수는 분기마다 바뀌므로 [endpoint auto scaling 문서](https://docs.aws.amazon.com/sagemaker/latest/dg/endpoint-auto-scaling.html)에서 **실행 전 재확인**하세요.
