...
**이 60초와 6 MB가 이 kit에 주는 실질적 제약**은 두 가지입니다.
...
- 대화형과 실시간 API가 목표이고 트래픽이 꾸준하다면 → **Real-time**을 선택하세요 (이 kit의 선택입니다).
...
[HF TGI](https://github.com/huggingface/text-generation-inference)와 HF PyTorch Inference DLC는 이 kit의 서빙 선택지에서 제외했습니다. 후자는 transformers 단건 서빙용이라 연속 배칭과 스트리밍이 없습니다(`dlc.resolve_hf_inference_image()`로 남겨 두긴 했습니다).
