# E2E 실행 가이드: 한 코스를 처음부터 끝까지 실행하기

!!! info "문서 범위"
    한 코스를 **AWS에서 처음부터 끝까지 실행**할 때 필요한 내용을 정리했습니다.

    - **선행 조건**: AWS 계정 + Amazon SageMaker AI 실행 role + Bedrock 모델 액세스.
      설치 절차와 smoke test, 로컬 dry-run은 [시작하기](getting_started.md).
...
!!! warning "빠르게 바뀌는 값"
    모델 ID, DLC 이미지 태그, SDK v3 API 이름, 인스턴스 타입과 리전별 GPU 가용성, AgentCore 상태는 자주 바뀝니다.
    이 문서의 태그, ID, 인스턴스 타입은 모두 **실행 직전에 다시 확인**해야 합니다. 코드에서는 `# TODO verify`로 표시했습니다.
    각 절에 연결한 공식 문서에서 최신 값을 확인하세요. 이 문서의 수치는 특정 시점의 측정값입니다.
...
**태스크 하나를 선택하고 `tracks/<track>/`의 노트북을 `00`부터 `99`까지 순서대로 실행합니다. 먼저 `pipelines/` 스크립트의 `--dry-run`이나 노트북의 `DRY_RUN=1`로 데이터와 핸드오프를 검증하세요. 단, 노트북의 `DRY_RUN=1`은 SageMaker AI Training Job과 endpoint 비용을 없애지 않습니다.**

폴더 이름과 코드 식별자(`tracks/`, `TRACKS`, `track_data.py`)는 초기 이름인 `track`을 그대로 쓰고 있습니다. 이 문서에서 말하는 **코스**와 같은 것을 가리킵니다.

1. **코스 하나가 독립된 E2E입니다.** 5개 코스는 서로 상태를 공유하지 않으므로 한 코스를 실행하고 리소스를 정리한 뒤 다음 코스로 이동하세요([5개 코스를 모두 실행하려면](#5개-코스를-모두-실행하려면)).
2. **핸드오프는 `%store`와 코스 로컬 파일로 이어집니다.** 어느 노트북이 무엇을 만들어 넘기는지는 [단계별 실행과 데이터 핸드오프](#단계별-실행과-데이터-핸드오프)의 표에 정리했습니다.
3. **작은 입력으로 먼저 검증합니다.** `DRY_RUN=1`로 데이터 준비와 평가 규모를 줄인 뒤, 문제가 없을 때 실제 규모로 실행하세요([DRY_RUN으로 먼저 검증하는 이유](#dry_run으로-먼저-검증하는-이유)).
4. **자주 발생하는 오류를 먼저 확인하세요.** `%store`에 남은 이전 값, `MaxRuntimeExceeded`, 24GB GPU CUDA OOM, Bedrock 모델 ID 형식 오류는 [E2E 실행 중 자주 발생하는 오류](#e2e-실행-중-자주-발생하는-오류)에 정리했습니다.
...
- "학습 Job이 `Completed`인데 **배포가 안 됩니다.**": 머지 단계가 시간 제한에 잘려 서빙용 모델이 artifact에 없습니다.
...
아래 절차에는 이 문제를 미리 확인하는 단계가 포함되어 있습니다.

---

## DRY_RUN으로 먼저 검증하는 이유

권한, 리전, 핸드오프, 데이터 스키마 문제는 데이터 양과 관계없이 발생합니다. 작은 입력으로 먼저 실행하면 비용과 대기 시간을 줄이면서 이런 문제를 확인할 수 있습니다.

| 구분 | 검증 실행 (`DRY_RUN=1`) | 실제 규모 실행 (`DRY_RUN=0`) |
|---|---|---|
| 목적 | 자격증명, 권한, 핸드오프, 스키마 확인 | 실제 데이터로 학습하고 성능 평가 |
| 시드, 합성 건수 | 시드 8건 / 합성 6건 | 시드 300건 / 합성 100~200건 |
| 학습 | 로컬 `train.py --dry_run`. SageMaker AI Job을 실행하려면 `MAX_TRAIN_SAMPLES`와 `EPOCHS`를 직접 낮춤 | SageMaker AI Training Job(`MAX_TRAIN_SAMPLES`, `EPOCHS`) |
| 평가 | `N_EVAL=20` | `N_EVAL` 기본 50 (env로 조절) |
| endpoint | 필요할 때만 배포하고 확인 후 즉시 삭제 | 평가와 agentic 단계가 끝나면 삭제 |
| 비용 | Bedrock 호출, Training Job, endpoint를 실행하면 비용 발생 | Training Job, endpoint, Bedrock 사용량에 따라 비용 발생 |

### DRY_RUN이 바꾸는 것 3가지

1. **합성과 시드 건수**: `01_data_and_synthetic`이 시드 8건, 합성 6건으로 줄입니다(`config.is_dry_run()`).
    일반 실행 값은 `NUM_SEED_SAMPLES=300`, `NUM_SYNTHETIC`(config 기본 200, 저장소의 `.env`는 100)입니다.
...
!!! warning "SageMaker AI 학습 Job 규모는 DRY_RUN이 줄이지 않습니다"
    `02_train_sft_sagemaker`가 제출하는 SageMaker AI Training Job의 크기는 노트북 안의 `MAX_TRAIN_SAMPLES`(hands-on 기본 200)와 `EPOCHS`(기본 2)가 결정합니다.
    `DRY_RUN`은 데이터 준비, 평가, 로컬 dry-run에만 적용되므로 Training Job 비용을 줄이려면 이 두 값을 직접 낮추세요.

`DRY_RUN=1`은 무료 실행 모드가 아닙니다. 비용이 발생하는 범위는 다음과 같습니다.

??? question "오해: “DRY_RUN=1이면 과금이 아예 없는 거죠?”"
    아닙니다. `DRY_RUN=1`은 일부 데이터와 평가 규모만 줄입니다. Bedrock 합성 호출에는 토큰 사용량에 따른 비용이 발생하고, `03_deploy_endpoint`를 실행하면 GPU endpoint가 삭제될 때까지 시간당 과금됩니다.
    AWS 리소스를 만들지 않는 검증은 `tests/test_smoke.py`, 로컬 GPU dry-run, `pipelines/` 스크립트의 `--dry-run`입니다.

---

## 두 가지 실행 방법

같은 코스를 **노트북**이나 **Python 스크립트**로 실행할 수 있습니다. 두 경로는 `common/` 코드를 공유하지만 상태를 전달하는 방식과 사용 목적이 다릅니다.

| | 노트북 (`tracks/`) | 스크립트 (`pipelines/`) |
|---|---|---|
| 적합 | 처음 배울 때, 중간 결과를 확인할 때, 질의를 바꿔가며 볼 때 | 검증된 코스를 다시 돌릴 때, CI, 무인 실행, 결과 재현 |
...
!!! tip "pipelines 스크립트는 --dry-run으로 먼저 확인"
    `pipelines/` 스크립트의 `--dry-run`은 Training Job과 endpoint를 만들지 않고 Bedrock도 호출하지 않습니다.
    AWS 자격증명 없이 실행 흐름과 상태 전달을 확인할 수 있습니다. 이는 노트북의 `DRY_RUN=1`과 다릅니다.
...
`train.jsonl`처럼 다른 코스의 값을 읽으면 안 되는 데이터는 코스 폴더 안의 파일로 저장합니다([단계별 실행과 데이터 핸드오프](#단계별-실행과-데이터-핸드오프) 표 참고).
...
00_setup ──▶ 01_data_and_synthetic ──▶ 02_train_sft_sagemaker ──▶ 03_deploy_endpoint
  (role,        (data/train.jsonl)         (model_data)              (endpoint_name)
   bucket)                                       │                         │
                        ...
                        (held-out 점수)   (SLM+Claude 루프)       (production 배포)
                                                                          │
                                                                          ▼
                                                                     99_cleanup
```

- **(선택) `02a_train_grpo_sagemaker`**(SFT→GRPO 정련)는 리워드를 프로그램으로 채점할 수 있는 **추출과 분류 코스에만** 있습니다.
- **(선택) `02b_local_serve`**(배포 전 로컬 vLLM preflight)는 4개 텍스트 코스 모두에 있습니다.
- 평가는 `04_evaluate`(로컬 메트릭, 빠름, 저렴) 한 경로입니다.

??? info "관리형 evaluator를 쓰지 않는 이유"
    SDK v3의 관리형 evaluator(`BenchMarkEvaluator`/`CustomScorerEvaluator`/`LLMAsJudgeEvaluator`)는 **SageMaker Public Hub에 평가 레시피가 등록된 모델(Amazon Nova, 일부 JumpStart)**을 대상으로 합니다.
    gemma-4 custom fine-tuning artifact(S3 checkpoint)에는 쓸 수 없습니다. 실측하면 `DescribeHubContent ... does not exist`로 실패합니다.

### 멀티모달 코스 (05) 파이프라인

멀티모달 코스는 **노트북 세트가 다르고 더 짧습니다**(이미지 입력, 합성과 agentic 단계 없음).
...
- [ ] **설치 완료**: `uv venv --python 3.12` → `uv pip install -r pyproject.toml`(자세한 절차는 [시작하기](getting_started.md)).
    주요 dependency는 최소 버전만 지정되어 있습니다(`sagemaker>=3.16.0`, `transformers>=5.14.1`, `trl>=1.8.0`, `peft>=0.19.1`). 현재 설정은 `pyproject.toml`에서 확인하세요.
    SDK v3는 클래스 이름이 v2와 다릅니다. 노트북 코드를 수정할 때는 [SageMaker Python SDK 저장소](https://github.com/aws/sagemaker-python-sdk)의 최신 API를 확인하세요.
...
- [ ] **SageMaker AI 실행 role**: `SAGEMAKER_ROLE_ARN`에 SageMaker AI, S3, ECR 권한이 있어야 합니다.
    ...
    **role을 찾았다고 해서 필요한 권한까지 확인된 것은 아닙니다.** 처음 실행하기 전에 S3와 ECR 권한을 확인하세요. 권한 부족은 Job 제출 후에 드러날 수 있습니다([실행 role이 매개하는 것](01_sagemaker_basics.md#실행-role로-무엇을-하는가-s3와-ecr-접근)).
...
VS Code에서 이 저장소를 workspace로 열면 `.env`의 설정값이 kernel env에 자동으로 주입됩니다. secret은 `.env`에 넣지 말고 shell export나 `hf auth login`을 사용하세요.
...
`jupyter lab`을 실행한 뒤 코스 폴더(`tracks/<track>/`)에서 노트북을 번호 순서대로 엽니다. 각 단계의 작업, 출력, 완료 조건은 다음과 같습니다.
...
| ③-b | (선택) `02b_local_serve` | 배포 전 로컬 vLLM preflight | (없음) | 로컬 invoke 응답 정상 |
| ④ | `03_deploy_endpoint` | [real-time endpoint](https://docs.aws.amazon.com/sagemaker/latest/dg/realtime-endpoints.html) 배포 + invoke smoke test | `%store`: `endpoint_name`, `ep_<track_key>` | `InService` 도달, invoke 응답 정상 |
...
| ⑦ | `06_agentcore_deploy` | [AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/what-is-bedrock-agentcore.html) 배포(production) | (없음) | (선택) Runtime 호출 성공 |
...
- **② 합성**: `NUM_SYNTHETIC`가 Bedrock 호출량, 즉 비용을 좌우합니다. config 기본값은 200이고, 저장소의 `.env`는 요약 코스 지연 때문에 100으로 낮춰 두었습니다.
...
- **③ 학습**: `stopping_condition`을 **반드시 명시**하세요. 생략하면 SDK 기본 1시간이 붙습니다.
    ...
    학습 이미지는 `.env`의 `DLC_IMAGE_URI`(완전 URI)를 `common/dlc.py`가 그대로 씁니다. env가 없으면 `DLC_REPOSITORY`+`DLC_TAG` 조립 → 라이브러리 버전 조합 순으로 fallback합니다.
...
- **④ 배포**: 기본 경로는 **vLLM DLC(`SERVING_ENGINE=vllm`)** 이고, `sglang`(같은 셀에서 처리) 또는 `lmi`(`OPTION_*` env)로 전환할 수 있습니다.
    ...
    endpoint는 한 번에 하나만 배포하세요. 여러 endpoint를 동시에 유지하면 각각 비용이 발생합니다. endpoint가 준비되기까지는 5~15분이 걸립니다.
...
- **⑥/⑦ agentic**: endpoint 사용 시간과 Bedrock 호출량에 각각 비용이 발생합니다.
...
!!! danger "평가셋은 합성으로 만들지 마세요"
    합성 데이터나 학습셋으로 평가하면 실제보다 높은 점수가 나올 수 있습니다. 이 경우 teacher 모델을 얼마나 모방했는지만 측정하게 됩니다.
...
??? info "더 읽을 거리"
    - [aws/deep-learning-containers](https://github.com/aws/deep-learning-containers): 학습과 서빙 DLC의 Dockerfile과 `sagemaker_entrypoint.sh`(env → CLI 플래그 변환 규칙)를 소스에서 확인할 때.
...
각 코스는 독립적으로 실행됩니다. 한 코스를 마치고 리소스를 정리한 뒤, 다른 코스 폴더에서 같은 순서를 반복하세요.
...
- 코스마다 **별도 endpoint**가 생성됩니다. 각 코스의 `99_cleanup`을 실행하세요.
- 공통 로직은 `common/`이 공유하므로, 텍스트 코스 간 차이는 데이터 어댑터(`tracks/*/track_data.py`)와 `config.TRACKS` 레지스트리뿐입니다.
- 여러 코스의 endpoint를 동시에 유지하면 GPU 인스턴스 비용도 늘어납니다. 한 코스씩 실행하고 정리하는 방식을 권장합니다.

??? question "오해: “코스를 옮기면 `%store` 값도 알아서 바뀌겠지?”"
    그렇지 않습니다. `%store`는 IPython 프로필 단위로 저장되므로 코스가 달라도 값이 공유됩니다. 전역 `endpoint_name`과 `model_data`에는 마지막으로 실행한 코스의 값이 남습니다. 이 때문에 다른 코스의 endpoint를 호출하거나 모델을 배포할 수 있습니다.
...
| 다른 코스 endpoint를 호출하거나 옛 모델이 배포됨 | `%store` 전역 키 오염 → 코스 전용 키(`ep_<track_key>`, `md_<track_key>`)를 쓰고, 리전을 바꿨다면 `aws_utils.ensure_model_data_in_region()`이 옛 리전 artifact를 걸러 줍니다 |
...
| 학습이 끝났는데 Job이 `Stopped`, artifact에 머지 모델이 없음 | `stopping_condition` 생략 시 붙는 SDK 기본 1시간(`MaxRuntimeExceeded`)에 머지 단계가 잘렸습니다 → `MAX_RUNTIME_HOURS`를 명시(기본 4시간). 실측에서는 Pending 6분 + Downloading 3분 + Training 55분(189 step 전부 완료) 후 머지 도중 종료됐고, `FailureReason`은 비어 있습니다. 상세는 [파인튜닝](03_finetuning.md) |
| `InsufficientInstanceCapacity`로 Job이 시작되지 않음 | 리전별 GPU 용량 문제 → `AWS_REGION`을 바꿔 재시도(`.env`의 DLC URI 리전도 함께 변경) |
| endpoint가 `Failed`, 이유는 `did not pass the ping health check`뿐 | CUDA OOM일 가능성이 큽니다. 24GB GPU(L4)에서 vLLM 기본 `max_num_seqs=256`은 샘플러 logits 버퍼에 `256 × 262,144 × 4B = 256 MiB`를 할당합니다 → `serving_env()` 기본값(`max_num_seqs=32`, `gpu_memory_utilization=0.90`)을 유지하고 CloudWatch endpoint 로그를 확인하세요([24GB GPU CUDA OOM](04_sagemaker_inference.md#24gb-gpu-cuda-oom-max_num_seqs-기본값)) |
...
| speculative decoding이 켜지지 않음 | 이 프로젝트 노트북에는 배선돼 있지 않습니다. 컨테이너 설정 키는 vLLM DLC `SM_VLLM_SPECULATIVE_CONFIG` / LMI `OPTION_SPECULATIVE_CONFIG`이며, **Gemma용 P-EAGLE head는 AWS가 공개하지 않았고** 커뮤니티 EAGLE3 head는 fine-tuned target과의 정합성을 직접 실측해야 합니다([Speculative decoding (EAGLE3 / P-EAGLE)](05_serving_containers.md#speculative-decoding-eagle3--p-eagle)) |
...
한 코스의 E2E 실행이 끝났는지 다음 항목으로 확인합니다.

- [ ] `02` 학습 Job이 `Completed`이고, artifact 루트에 **머지된 서빙용 모델**이 있음(어댑터만 있으면 배포되지 않습니다)
- [ ] `03` invoke smoke test가 의미 있는 출력을 반환
- [ ] `04_evaluate` 지표가 나옴 (가능하면 파인튜닝 전 baseline과 비교해 개선 폭 확인)
- [ ] (선택) `05_agentic_strands`에서 Claude가 SLM endpoint를 tool로 호출하는 왕복이 성공
- [ ] (production 목표 시) `06_agentcore_deploy`로 Runtime 배포 확인
- [ ] `99_cleanup` 실행 후 이 코스 prefix의 endpoint가 콘솔에 남아 있지 않음
...
!!! danger "비용과 cleanup"
    **real-time endpoint는 삭제하기 전까지 시간당(GPU 인스턴스) 요금이 계속 부과됩니다.** 호출이 0건이어도 켜져 있는 동안 과금됩니다.
    실습이 끝나면 **실행한 코스의 `99_cleanup`을 실행하고 콘솔에 endpoint가 남아 있지 않은지 확인**하세요.
    여러 번 배포했다면 `%store`의 `endpoint_name`은 마지막 값만 가리킵니다. 코스 prefix(`gemma-extraction` 등)로 남은 리소스를 검색하고, 다른 리전에서도 배포했다면 해당 리전도 확인하세요.
...
각 노트북은 학습과 배포 직후 **CloudWatch 다이렉트 링크**를 출력합니다(`common/aws_utils.cw_links()`). Job 로그, endpoint 기동, OOM, Bedrock 호출량을 여기서 실시간으로 볼 수 있습니다.

??? question "오해: “endpoint를 호출하지 않으면 요금도 안 나오죠?”"
    **아닙니다.** real-time endpoint는 호출 여부와 무관하게 **provisioned 인스턴스가 시간당** 과금됩니다. auto scaling을 설정해도 통상 최소 1대는 유지합니다.
    사용하지 않는 endpoint는 삭제해야 합니다. 비용 관점의 전체 비교는 [비용과 cleanup](04_sagemaker_inference.md#비용과-cleanup)에 있습니다.
...
- `common/dlc.py`: DLC 이미지 URI 해석(`DLC_IMAGE_URI` → `DLC_REPOSITORY`+`DLC_TAG` → 버전 조합 fallback)과 서빙 env 생성(`serving_env`)
...
- `tracks/*/scripts/train_grpo.py`: SFT artifact를 reward 함수로 정련하는 GRPO 학습(추출과 분류 코스만)
