...
**이 kit은 뒤쪽 경우입니다.** vLLM/SGLang/LMI DLC를 이미지로 지정해 넘기므로 자동 탐지가 개입하지 않습니다([서빙 컨테이너](../05_serving_containers.md)에 그 이미지들과 고정 태그가 있습니다).
...
V3는 이 둘을 분리해서 다룹니다. 이 kit을 읽다 보면 "vLLM DLC"와 "vLLM 서버"가 섞여 보이는데, 층이 다릅니다.
