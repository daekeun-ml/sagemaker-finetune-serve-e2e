...
- 이 kit의 기본값은 `common/config.py`의 `BEDROCK_CLAUDE_MODEL_ID = "global.anthropic.claude-sonnet-5"`이며, 이 값은 **해당 계정에서 `list_inference_profiles`로 확인**한 것입니다(실측 2026-07). 최신 Claude는 날짜 없는 pinned-snapshot 형식을 씁니다.
