...
!!! tip "SageMaker AI의 핵심 가치"
    **ML 인프라를 직접 운영하는 부담을 줄이고 모델 개발과 배포에 집중하게 하는 것**입니다. 사용자는 실행할 코드, 데이터, 컨테이너, 인스턴스 유형과 권한을 정하고, SageMaker AI는 필요한 컴퓨팅을 준비해 작업을 실행하며 S3와 CloudWatch에 결과와 로그를 연결합니다.

    쉽게 말해 GPU 서버를 먼저 만들고 환경을 하나씩 구성하는 대신, **작업을 제출하면 필요한 실행 환경이 준비됩니다.**
