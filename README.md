...
SageMaker AI가 처음이어도 읽을 수 있게 개념부터 시작하고, 각 절이 **어디서 막히는지(pain point) → 왜 그런지(why) → 그래서 이 값과 이 구조인지**로 이어집니다.
...
몇 초 안에 끝나고 **과금되는 것을 하나도 만들지 않습니다.** 학습 Job과 endpoint는 물론 Bedrock 호출도 하지 않으므로, AWS 자격증명이 없어도 전 경로를 밟아 볼 수 있습니다.
...
> [!WARNING]
> **real-time endpoint는 삭제할 때까지 시간당 과금됩니다.** 요청이 없어도 인스턴스 비용이 발생합니다.
...
- 자체 데이터로 SLM을 파인튜닝해 **endpoint까지 올려 보고 싶은 분**. 데이터 준비부터 정리까지 한 코스면 끝납니다.
...
| Item | Notes |
|---|---|
| AWS 계정 | SageMaker AI 학습 Job과 endpoint를 만들 수 있는 계정 (과금 발생) |
...
**비용 감각.** 기본값(200건 x 2 epoch)이면 학습 1회가 20분대이고, endpoint는 시간당 과금입니다. 코스 하나를 학습부터 정리까지 도는 데 대략 몇 달러 수준입니다(리전과 인스턴스에 따라 다름). **endpoint를 삭제하지 않으면 계속 과금되므로** `99_cleanup.ipynb`를 반드시 실행하세요.
...
| 학습 | PyTorch DLC + TRL `SFTTrainer` + PEFT LoRA/QLoRA. 추출과 분류 코스는 SFT→GRPO 정련 선택 가능 |
| 서빙 | SageMaker AI real-time endpoint. vLLM(기본) / SGLang / DJL LMI 중 선택 (셋 다 OpenAI 호환) |
| 평가 | held-out 세트로 endpoint를 직접 호출해 코스별 지표 산출 (합성 데이터와 학습셋 사용 금지) |
...
학습 스크립트(`scripts/train.py`)는 self-contained이며 **로컬 GPU `--dry_run`과 SageMaker AI 학습 Job에서 같은 파일**을 씁니다. 클라우드에 제출하기 전에 로컬에서 파이프라인을 먼저 검증할 수 있습니다.
...
- `02a_train_grpo_sagemaker` (선택): 추출과 분류 코스만. reward를 프로그램으로 채점할 수 있는 태스크에 적용합니다.
...
각 코스는 노트북 + `track_data.py`(시드 어댑터) + `scripts/`(학습과 서빙 코드)로 같은 구조를 갖습니다. 디렉터리 이름 `tracks/`, 그리고 `track_data.py`와 `TRACKS` 같은 코드 식별자는 초기 명칭을 그대로 유지하고 있으므로, 본문의 "코스"와 코드의 `track`은 같은 것을 가리킵니다.
...
| `scripts/train.py` | SFT. 로컬 `--dry_run`과 SageMaker AI 학습 Job에서 같은 파일을 씁니다 |
...
`config.yaml`은 값의 **이유를 주석으로** 담고 있어서, 왜 `max_num_seqs`가 32인지 파일 안에서 알 수 있습니다. 우선순위는 **셸과 `.env`의 기존 env > `config.yaml` > `common/config.py` 기본값**이라, 한 번만 바꿀 때는 셸에서 넘기면 됩니다.
...
- 학습 Job은 종료 시 과금이 멈춥니다. 노트북이 `stopping_condition`으로 상한을 걸어 두므로 Job이 무한히 도는 일은 없습니다.
- Bedrock Converse는 토큰 단위 과금입니다 (대량 합성 시 비용 발생). AgentCore Runtime도 배포해 두면 과금됩니다.
- 각 노트북은 학습과 배포 직후 CloudWatch 다이렉트 링크를 출력합니다.
